...
//!
//! // 为用户生成魔法链接 token
//! let token_data = manager.generate("user@example.com").unwrap();
//!
//! // 构建完整的魔法链接 URL（应用层负责）
...
//!
//! // 用户点击链接后，验证 token
//! match manager.verify(&token_data.token) {
...
//!
//! // 用户输入验证码后，验证 OTP
//! match manager.verify("user@example.com", "123456", OtpPurpose::Login) {
//!     Ok(()) => println!("验证成功"),
//!     Err(e) => println!("验证失败: {}", e),
//! }
//! ```
